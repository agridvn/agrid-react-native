...
import { AgridRNStorage, AgridRNSyncMemoryStorage } from './storage'
import { version } from './version'
import { buildOptimisiticAsyncStorage, getAppProperties } from './native-deps'
import {
  AgridAutocaptureOptions,
  AgridCustomAppProperties,
  AgridCustomStorage,
  AgridSessionReplayConfig,
} from './types'
...
export interface AgridOptions extends PostHogCoreOptions {
  ...
  customAppProperties?:
    | AgridCustomAppProperties
    | ((properties: AgridCustomAppProperties) => AgridCustomAppProperties)
...
export class Agrid extends PostHogCore {
  private _persistence: AgridOptions['persistence']
  private _storage: AgridRNStorage
  private _appProperties: AgridCustomAppProperties = {}
  ...
  constructor(apiKey: string, options?: AgridOptions) {
    ...
    if (this._persistence === 'file') {
      this._storage = new AgridRNStorage(options?.customStorage ?? buildOptimisiticAsyncStorage())
      storagePromise = this._storage.preloadPromise
    } else {
      this._storage = new AgridRNSyncMemoryStorage()
    }

    if (storagePromise) {
      storagePromise.catch((error) => {
        console.error('Agrid storage initialization failed:', error)
      })
    }
  ...
  private async startSessionReplay(options?: AgridOptions): Promise<void> {
  ...
  private async captureAppLifecycleEvents(): Promise<void> {
    ...
    if (!isMemoryPersistence) {
      ...
      if (!appBuild || !appVersion) {
        this._logger.warn(
          'Agrid could not track installation/update/open, as the build and version were not set. ' +
      ...
      this._logger.warn(
        'Agrid was initialised with persistence set to "memory", capturing native app events (Application Installed and Application Updated) is not supported.'
      )
    }
