...
const POSTHOG_STORAGE_KEY = '.agrid-rn.json'
const POSTHOG_STORAGE_VERSION = 'v1'

type AgridStorageContents = { [key: string]: any }

export class AgridRNStorage {
  ...
  populateMemoryCache(res: string | null): void {
    try {
      ...
      console.warn(
        "Agrid failed to load persisted data from storage. This is likely because the storage format is. We'll reset the storage.",
        e
      )
    }
  }
...
export class AgridRNSyncMemoryStorage extends AgridRNStorage {
  constructor() {
