...
export function useActivatedSurveys(agrid: Agrid, surveys: Survey[]): ReadonlySet<string> {
  ...
  useEffect(() => {
    if (eventMap.size > 0) {
      return agrid.on('capture', (payload: { event: string; properties?: { $survey_id?: string } }) => {
