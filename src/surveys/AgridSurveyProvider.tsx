...
import { Survey, SurveyAppearance, SurveyType } from '@agrid/core'
...
export function AgridSurveyProvider(props: AgridSurveyProviderProps): JSX.Element {
  const agridFromHook = useAgrid()
  const agrid = props.client ?? agridFromHook
  ...
  const activatedSurveys = useActivatedSurveys(agrid, surveys)

  const flags = useFeatureFlags(agrid)

  // Load surveys once
  useEffect(() => {
    agrid
      .ready()
      .then(() => agrid._onSurveysReady())
      .then(() => agrid.getSurveys())
      .then(setSurveys)
      .catch(() => {})
  }, [agrid])
  ...
  const surveyAppearance = useMemo<SurveyAppearanceTheme>(() => {
    ...
    return {
      ...
      // If submitButtonColor is set by Agrid, ensure submitButtonTextColor is also set to contrast
  ...
  const activeContext = useMemo(() => {
    ...
    return {
      survey: activeSurvey,
      onShow: () => {
        sendSurveyShownEvent(activeSurvey, agrid)
      ...
      onClose: (submitted: boolean) => {
        ...
        if (!submitted) {
          dismissedSurveyEvent(activeSurvey, agrid)
        }
      },
    }
  }, [activeSurvey, agrid, setLastSeenSurveyDate, setSeenSurvey])
