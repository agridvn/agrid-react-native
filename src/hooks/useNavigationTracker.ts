...
import { AgridAutocaptureNavigationTrackerOptions } from '../types'
import { useAgrid } from './useAgrid'
import { AgridNavigationRef } from '../types'
...
function _useNavigationTracker(
  options?: AgridAutocaptureNavigationTrackerOptions,
  ...
  const trackRoute = useCallback((): void => {
    ...
    if (currentRouteName) {
      ...
      agrid.screen(currentRouteName, properties)
    }
  }, [navigation, options, agrid])
